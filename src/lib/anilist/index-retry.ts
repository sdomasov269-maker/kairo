...
export type AniListIndexRetryOptions = {
  maxRetries: number;
...
export async function loadAniListIndexPageWithRetry<T>(
  page: number,
  load: (page: number) => Promise<T>,
  options: AniListIndexRetryOptions,
): Promise<T> {
  const sleep =
    options.sleep ??
  ...
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      const exponential = Math.min(
        options.backoffBaseMs * 2 ** attempt,
        options.backoffMaxMs,
      );
      const jitter = Math.floor(
        exponential * 0.2 * (options.jitter?.() ?? Math.random()),
      );
      await sleep(
