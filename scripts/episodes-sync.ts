...
} from "../src/domain/watch/episode-schedule.ts";

const prisma = new PrismaClient();

async function refreshAnimeSchedule(
  anime: Anime,
  persist: boolean,
): Promise<Anime> {
  ...
  return persist
    ? prisma.anime.update({ where: { id: anime.id }, data: changes })
    : { ...anime, ...changes };
}

async function syncAnime(input: Anime, options: EpisodeSyncArguments) {
  const anime = options.refreshSchedule
    ? await refreshAnimeSchedule(input, !options.dryRun)
    : input;
  ...
  let missing = planMissingEpisodes(
    anime,
    current.map((episode) => episode.number),
  );
  if (options.refreshSchedule)
    missing = missing.filter(
      (episode) => episode.number === anime.nextAiringEpisode,
    );
  ...
  const reason = !expected
    ...
    : (limitReason ?? (missing.length ? null : "already-complete"));
  const wouldCreateSeason = seasonCount === 0 && missing.length > 0 ? 1 : 0;

  if (missing.length) {
    await executeEpisodeWrite(options.dryRun, () =>
...
async function main() {
  ...
  if (
    (options.allMissing || options.allOngoing) &&
    !options.apply &&
    !process.argv.includes("--dry-run")
  ) {
    console.warn("Mass write requires --apply; continuing in dry-run mode.");
  }
  const after = options.afterAniListId
    ? { anilistId: { gt: options.afterAniListId } }
    : {};
  ...
  const anime = await prisma.anime.findMany({
    ...
    take: options.anilistId ? 1 : options.limit,
  });
  if (!anime.length)
    throw new Error(
      options.anilistId
        ? `Anime ${options.anilistId} not found`
  ...
  for (const item of anime) {
    ...
    const [result] = await executePlansIndependently([item], (candidate) =>
      syncAnime(candidate, options),
    );
    if (result.ok) reports.push(result.value);
      ...
      const kind =
        error instanceof ScheduleRefreshError ? error.kind : "UNKNOWN";
      console.warn(`[episodes:sync] ${item.anilistId} ${kind}`);
      if (kind === "HTTP_429") rateLimited = true;
    }
    if (options.refreshSchedule && !rateLimited)
  ...
  console.table(
    reports.map((report) => ({
      ...
      savedEpisodes: report.savedEpisodes,
      existingSeasons: report.existingSeasons,
      existingEpisodes: report.existingEpisodes,
      createSeasons: report.createSeasons,
      createEpisodes: report.createEpisodes,
      reason: report.reason,
    })),
  );
  const summary = {
    Processed: reports.length,
    "Would create seasons": reports.reduce(
      (sum, report) => sum + report.createSeasons,
      0,
    ),
    "Would create episodes": reports.reduce(
      (sum, report) => sum + report.createEpisodes,
      0,
    ),
    Skipped: reports.filter((report) => report.reason).length,
    "Unknown episode count": reports.filter(
      (report) => report.reason === "episode-count-unknown",
    ).length,
    Movies: reports.filter((report) => report.movie).length,
    Ongoing: reports.filter((report) => report.ongoing).length,
    Errors: errors,
    dryRun: options.dryRun,
    checkpoint: nextEpisodeCheckpoint(
      reports.map((report) => report.anilistId),
      options.afterAniListId,
    ),
