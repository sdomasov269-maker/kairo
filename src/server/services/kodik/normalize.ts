...
import type { KodikBlockedSeasons, NormalizedKodikSeason } from "./types.ts";
...
export function isKodikEpisodeBlocked(
  ...
  return (
    Array.isArray(blocked) && blocked.some((value) => Number(value) === episode)
  );
}
...
export function normalizeKodikSeasons(
  ...
  const seasons = Object.entries(material.seasons ?? {}).flatMap(
    ([seasonKey, value]) => {
      const number = Number(seasonKey);
      if (!Number.isSafeInteger(number) || number < 0) return [];
      const playerLink = value.link
        ? (normalizePlayerLink(value.link) ?? undefined)
        : undefined;
