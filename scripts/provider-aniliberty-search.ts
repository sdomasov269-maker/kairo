...
async function main() {
  ...
  const result = await new AniLibertyClient().searchTitlesWithDiagnostics(
    query,
  );
  console.log(
    JSON.stringify(result.items.map(mapAniLibertySearchItem), null, 2),
  );
  console.log(
...
await main().catch((error: unknown) => {
  if (error instanceof AniLibertySchemaError)
    console.error(`AniLiberty schema error: ${error.message}`);
  else
    console.error(
      error instanceof Error ? error.message : "AniLiberty search failed",
    );
  process.exitCode = 1;
});
