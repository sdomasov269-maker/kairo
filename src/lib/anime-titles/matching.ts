...
export function scoreAnimeCandidate(
  ...
  candidate: ExternalAnimeCandidate,
): number {
  let score = 0;
  if (same(input.native, candidate.native)) score += 45;
  const primaryMatch = Math.max(
    same(input.romaji, candidate.romaji) ||
      same(input.romaji, candidate.english)
      ? 35
      : 0,
    same(input.english, candidate.english) ||
      same(input.english, candidate.romaji)
      ? 35
      : 0,
  );
  score += primaryMatch;
  ...
  if (inputSynonyms.some((title) => candidateTitles.includes(title)))
    score += 25;
  if (input.startYear && candidate.year)
    score += input.startYear === candidate.year ? 15 : -30;
...
export function rankAnimeCandidates(
  input: AnimeTitleCandidateInput,
  candidates: ExternalAnimeCandidate[],
  minConfidence = 80,
) {
  const ranked = candidates
    .map((candidate) => ({
      candidate,
      score: scoreAnimeCandidate(input, candidate),
    }))
    .sort((a, b) => b.score - a.score);
  const best = ranked[0];
  if (!best || best.score < minConfidence)
    return { status: "not-found" as const, ranked };
  if (ranked[1] && best.score - ranked[1].score < 8)
    return { status: "ambiguous" as const, ranked };
