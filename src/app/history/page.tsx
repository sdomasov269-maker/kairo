...
export default function HistoryPage() {
  ...
  const items = useMemo(
    () =>
      progress
        .filter((item) => item.animeSlug.includes(filter.trim().toLowerCase()))
        ...
        <PageContainer className="account-collection-container">
          ...
          {items.length ? (
            <div className="account-card-grid history-grid">
              {items.map((item) => (
                <article
                  className="account-item-card history-card"
                  key={`${item.animeSlug}-${item.seasonNumber}-${item.episodeNumber}`}
                >
                  <div className="history-card-art" aria-hidden="true">
                  ...
                  <div className="account-item-copy">
                    ...
                    <div
                      ...
                      aria-label={`${Math.round(item.percent)}%`}
                    >
                      <i
                        style={{
