...
try {
  ...
  const [anime, localizedTitles, lookupCache] = await Promise.all([prisma.anime.count(), prisma.animeLocalizedTitle.count(), prisma.animeTitleLookupCache.count()]);
  console.log(JSON.stringify({ migrations, columns: columns.map((row) => row.column_name), enumValues: enumValues.map((row) => row.enumlabel), counts: { anime, localizedTitles, lookupCache } }, null, 2));
} finally {
