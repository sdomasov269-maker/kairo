...
export class ShikimoriTitleProvider implements AnimeTitleProvider {
  ...
  constructor(
    ...
    baseUrl = process.env.SHIKIMORI_API_BASE_URL || "https://shikimori.one/api",
  ) {
  ...
  async findTitles(anime: AnimeTitleCandidateInput): Promise<ProviderLookup> {
    if (process.env.ANIME_TITLES_SHIKIMORI_ENABLED === "false")
      return { status: "not-found", results: [] };
    if (!anime.malId)
      return {
        status: "not-eligible",
        results: [],
        diagnostics: { reason: "not-eligible-without-mal-id" },
      };
    try {
      let candidates: ShikimoriAnime[] = [];
      let exactMalMatch = false;
      if (anime.malId) {
        const exact = await fetchJson<ShikimoriAnime>(
          `${this.baseUrl}/animes/${anime.malId}`,
          this.fetcher,
        );
      ...
      if (!candidates.length) {
        ...
        candidates = await fetchJson<ShikimoriAnime[]>(
          `${this.baseUrl}/animes?search=${encodeURIComponent(query)}&limit=10`,
          this.fetcher,
        );
      }
      const mapped: ExternalAnimeCandidate[] = candidates.map((item) => ({
        ...
        native: item.japanese?.[0],
        synonyms: item.synonyms ?? [],
        year: item.aired_on ? Number(item.aired_on.slice(0, 4)) : null,
        format: item.kind,
      ...
      if (exactMalMatch && candidates[0]?.russian?.trim()) {
        ...
        return {
          ...
          results: [
            {
              title: source.russian!.trim(),
              locale: "ru",
              source: "SHIKIMORI",
              confidence: 100,
              externalId: String(source.id),
              aliases: source.synonyms ?? [],
              metadata: {
                malId: source.myanimelist_id,
                match: "mal-id",
                score: 100,
              },
            },
      ...
      const source = candidates.find(
        (item) => String(item.id) === ranked.best.candidate.id,
      )!;
      if (!source.russian?.trim())
        return {
          status: "not-found",
          results: [],
          diagnostics: { reason: "empty-russian-title" },
        };
      return {
        status: "found",
        results: [
