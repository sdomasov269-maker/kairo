...
function sanitize(value: unknown, key = ""): unknown {
  if (Array.isArray(value))
    return value.slice(0, 3).map((item) => sanitize(item));
  ...
  if (
    /(token|cookie|authorization|signature|external_player|hls|url)/i.test(key)
  )
    return "[REDACTED]";
  if (/^(?:https?:)?\/\//i.test(value))
    return "https://example.invalid/redacted";
  if (key === "description" && value.length > 150)
...
async function main() {
  ...
  const url = new URL("https://aniliberty.top/api/v1/app/search/releases");
  ...
  if (!response.ok)
    throw new Error(`Search inspect failed with HTTP ${response.status}`);
  ...
  for (const [index, item] of results.slice(0, 3).entries()) {
    ...
    console.log("Enum-like:", {
      type: sanitize(record.type),
      season: sanitize(record.season),
      status: sanitize(record.status),
    });
  }
  const fixture = sanitize(results.slice(0, 3));
