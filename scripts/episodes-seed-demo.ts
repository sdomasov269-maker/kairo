...
async function main() {
  const anime = await prisma.anime.upsert({
    where: { slug: "eclipse-protocol" },
    create: {
      slug: "eclipse-protocol",
      anilistId: 999999001,
      titleEnglish: "Eclipse Protocol",
  ...
  for (const number of [1, 2, 3])
    await prisma.animeEpisode.upsert({
      ...
      create: {
        ...
        titleRu: `Серия ${number}`,
        titleUk: `Серія ${number}`,
        durationSec: 596,
        isPublished: true,
        introStartSec: number === 1 ? 0 : null,
        introEndSec: number === 1 ? 12 : null,
      },
      update: {},
    });
  ...
  await prisma.animeVideoSource.upsert({
    where: { id: "eclipse-db-dash" },
    create: {
      id: "eclipse-db-dash",
