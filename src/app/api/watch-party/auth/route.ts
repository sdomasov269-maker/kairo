...
export async function GET(request: Request) {
  try {
    ...
    const room = await resolveWatchParty(
    ...
    const limit = Math.max(
      2,
      Math.min(50, Number(process.env.WATCH_PARTY_MAX_MEMBERS) || 10),
    );
    const presenceResponse = await fetch(
      `https://rest.ably.io/channels/${encodeURIComponent(room.channelName)}/presence?limit=${limit + 1}`,
      {
        headers: {
    ...
    if (presenceResponse.ok) {
      ...
      const members = new Set(
        (presence.items ?? []).map((item) => item.clientId).filter(Boolean),
      );
      if (!members.has(user.userId) && members.size >= limit)
