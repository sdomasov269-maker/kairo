...
async function main() {
  const now = new Date();
  const [
    animeTotal, animeWithSeasons, animeWithEpisodes, seasons, episodes,
    movies, tv, ova, ona, special, ongoing, knownCount,
    comingSoon, unpublished, noVideo, futureAvailable,
    ...
    prisma.animeEpisode.count({ where: { isPublished: true, availableAt: { gt: now } } }),
    prisma.animeEpisode.count({ where: { isPublished: false } }),
    prisma.animeEpisode.count({ where: { isPublished: true, OR: [{ availableAt: null }, { availableAt: { lte: now } }], videoSources: { none: { isActive: true } } } }),
  ...
  if (args.has("largest")) {
    ...
    const largest = await prisma.anime.findMany({
      where: { episodes: { not: null } }, orderBy: { episodes: "desc" }, take: limit,
      select: { anilistId: true, slug: true, format: true, status: true, episodes: true, _count: { select: { animeEpisodes: true } } },
    });
    console.table(largest.map((anime) => ({
      anilistId: anime.anilistId, slug: anime.slug, format: anime.format, status: anime.status,
      episodes: anime.episodes, existingEpisodes: anime._count.animeEpisodes,
      plannedPlaceholders: Math.max(0, (anime.episodes ?? 0) - anime._count.animeEpisodes),
    })));
  }
}
