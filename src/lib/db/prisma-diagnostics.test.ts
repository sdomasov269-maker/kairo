...
import {
  ...
  warnOnce,
} from "./prisma-diagnostics.ts";

test("Prisma delegate assertion reports a stale generated client", () => {
  assert.throws(
    () => assertPrismaDelegate({ anime: {} }, "animeLocalizedTitle"),
    /Run prisma generate and restart the server/,
  );
  assert.doesNotThrow(() =>
    assertPrismaDelegate(
      { animeLocalizedTitle: { findMany() {} } },
      "animeLocalizedTitle",
    ),
  );
});
test("Prisma system warnings are emitted once per failure kind", () => {
  ...
  assert.equal(
    classifyPrismaFailure({ code: "P2021", message: "missing" }).kind,
    "TABLE_MISSING",
  );
});
