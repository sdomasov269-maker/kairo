...
import {
  localizePublicAnimeListWithLoader,
  normalizeAnimeTitleLocale,
} from "./public-list.ts";

const anime = (overrides: Partial<Anime> = {}): Anime => ({
  id: "anilist-21",
  anilistId: 21,
  slug: "anilist-21-one-piece",
  title: "One Piece",
...
test("the same AniList ID gets one RU displayTitle across public sections", async () => {
  ...
  const loader = async () => {
    calls += 1;
    return new Map([[21, { ru: "Ван-Пис", aliases: [] }]]);
  };
  for (const section of ["home", "new", "catalog"]) {
    const [card] = await localizePublicAnimeListWithLoader(
      [anime({ id: section })],
      "ru",
      loader,
    );
...
test("catalog fallbacks are RU, English, Romaji, then Native", async () => {
  ...
  assert.equal(
    (
      await localizePublicAnimeListWithLoader(
        [anime()],
        "ru",
        async () => new Map(),
      )
  ...
  assert.equal(
    (
      await localizePublicAnimeListWithLoader(
        [anime({ title: "", titleEnglish: undefined })],
        "ru",
        async () => new Map(),
      )
    )[0].displayTitle,
  ...
  assert.equal(
    (
      await localizePublicAnimeListWithLoader(
        [anime({ title: "", titleEnglish: undefined, titleRomaji: undefined })],
        "ru",
        async () => new Map(),
      )
    )[0].displayTitle,
    ...
    assert.equal(
      (
        await localizePublicAnimeListWithLoader(
          page.filter(() => true),
          "ru",
          async () => new Map([[21, { ru: "Ван-Пис", aliases: [] }]]),
        )
      )[0].displayTitle,
      "Ван-Пис",
    );
...
test("English base data cannot overwrite displayTitle and locale results stay separate", async () => {
  const load = async () =>
    new Map([[21, { ru: "Ван-Пис", uk: "Ван-Піс", aliases: [] }]]);
  const [ru] = await localizePublicAnimeListWithLoader(
    [anime()],
    "ru-RU",
    load,
  );
  const [en] = await localizePublicAnimeListWithLoader(
    [anime()],
    "en-US",
    load,
  );
...
test("twenty cards perform one batch loader call", async () => {
  ...
  const cards = Array.from({ length: 20 }, (_, index) =>
    anime({ id: String(index), anilistId: index + 1 }),
  );
  await localizePublicAnimeListWithLoader(cards, "ru", async () => {
    calls += 1;
    return new Map();
  });
