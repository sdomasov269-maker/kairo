...
test("only-missing is a Prisma relation filter applied before take", () => {
  assert.deepEqual(buildMissingTitleWhere("ru"), {
    localizedTitles: { none: { locale: "RU" } },
  });
  const anime = Array.from({ length: 4206 }, (_, index) => ({
    hasRu: index < 2884,
  }));
...
test("429 is retryable and empty result is permanent NOT_FOUND", () => {
  assert.deepEqual(
    classifyLookup({
      ...
      results: [],
      diagnostics: { status: 429, error: "HTTP 429" },
    }),
    {
      status: "RATE_LIMITED",
      retryable: true,
  ...
  assert.equal(
    classifyLookup({ status: "not-found", results: [] }).status,
    "NOT_FOUND",
  );
  assert.equal(
    classifyLookup({
      status: "temporary-error",
      results: [],
      diagnostics: { status: 404, error: "HTTP 404" },
    }).status,
    "NOT_FOUND",
  );
...
test("successful retry replaces the classified error and circuit stops only future work", () => {
  assert.equal(
    classifyLookup({ status: "found", results: [] }).status,
    "FOUND",
  );
  const breaker = new LookupCircuitBreaker(10);
  for (let index = 0; index < 9; index += 1)
    assert.equal(breaker.record("SERVER_ERROR"), false);
...
test("Wikidata RU requires explicit opt-in", () => {
  assert.equal(
    wikidataRuEnabled({ fallbackWikidata: false, skipWikidata: false }),
    false,
  );
  assert.equal(
    wikidataRuEnabled({ fallbackWikidata: true, skipWikidata: false }),
    true,
  );
});
