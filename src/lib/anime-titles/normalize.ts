...
export function buildAnimeTitleVariants(input: AnimeTitleCandidateInput): string[] {
  const values = [input.native, input.romaji, input.english, ...input.synonyms];
  return [...new Set(values.map((value) => value && normalizeAnimeTitle(value)).filter((value): value is string => Boolean(value)))];
}
