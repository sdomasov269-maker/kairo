...
import {
  applyAniLibertySync,
  prepareAniLibertySync,
} from "@/server/media-providers/adapters/aniliberty/sync";
...
export async function POST(request: Request) {
  ...
  const { release, plan } = await prepareAniLibertySync(
    prisma,
    parsed.data.releaseId,
  );
  const apply = parsed.data.confirm === true;
  console.info("[audit] AniLiberty admin import", {
    userId: session.userId,
    releaseId: String(parsed.data.releaseId),
    apply,
  });
  if (!apply) return NextResponse.json({ dryRun: true, plan });
  return NextResponse.json({
    dryRun: false,
    result: await applyAniLibertySync(prisma, release, plan),
  });
}
