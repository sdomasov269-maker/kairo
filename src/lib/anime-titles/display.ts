export type AnimeBaseTitles = { romaji: string | null; english: string | null; native: string | null };
...
export function resolveDisplayAnimeTitle(params: {
  ...
  return candidates.map(clean).find((value): value is string => Boolean(value)) ??
    (locale === "ru" ? "Название неизвестно" : locale === "uk" ? "Назва невідома" : "Unknown title");
}
