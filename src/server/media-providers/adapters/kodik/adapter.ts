...
export class KodikMediaProvider implements AnimeMediaProvider {
  readonly key = "kodik";
  readonly name = "Kodik";
  readonly capabilities = kodikCapabilities;
  private readonly client: KodikClient;
  constructor(client = new KodikClient()) {
    this.client = client;
  }
  async healthCheck() {
    return {
  ...
  ): Promise<ProviderPlayback | null> {
    void input;
    throw new KodikPartnerAccessRequiredError(
      "Kodik playback is disabled pending written permission",
    );
  }
}
export function createKodikProviderIfAuthorized() {
  return null;
}
